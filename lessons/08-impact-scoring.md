...
## Цель урока
Оценивать direction и strength по evidence-backed rubric.

## Входное состояние проекта
Файлы, которые ученик должен открыть перед занятием: `src/news_impact/scoring.py, evals/expected_labels.jsonl`.
...
## Ожидаемый результат
Экспортные ограничения дают down/strong, HBM supply дает up/strong.
