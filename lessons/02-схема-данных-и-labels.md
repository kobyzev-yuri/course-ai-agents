...
## Цель урока
Закрепить доменную модель: Article, Story, ImpactAssessment и допустимые labels.

## Входное состояние проекта
Файлы, которые ученик должен открыть перед занятием: `src/news_impact/schema.py, tests/test_schema.py`.
...
## Ожидаемый результат
Все labels валидируются тестами.
