...
## Цель урока
Связать тип события с горизонтом 1w/3w/5w/1m.

## Входное состояние проекта
Файлы, которые ученик должен открыть перед занятием: `src/news_impact/scoring.py`.
...
## Ожидаемый результат
Ученик объясняет отличие immediate catalyst от slow-burn catalyst.
