...
## Цель урока
Собрать ranked report для аналитика: что важно, почему, на какой горизонт.

## Входное состояние проекта
Файлы, которые ученик должен открыть перед занятием: `src/news_impact/reporting.py, src/news_impact/cli.py`.
...
## Ожидаемый результат
CLI выводит markdown/jsonl report без секрета и без торговых советов.
