...
## Цель урока
Использовать crew как экспертную проверку, а не как магический оракул.

## Входное состояние проекта
Файлы, которые ученик должен открыть перед занятием: `src/news_impact/crewai_review.py`.
...
## Ожидаемый результат
Review layer выдает flags и не подменяет evidence.
