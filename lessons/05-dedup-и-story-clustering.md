# Урок 5. Dedup и story clustering
...
## Цель урока
Отличать stories от article copies, чтобы не завышать важность перепечаток.

## Входное состояние проекта
Файлы, которые ученик должен открыть перед занятием: `src/news_impact/dedup.py`.
...
## Ожидаемый результат
Повторы Blackwell-новости объединяются в одну story.
