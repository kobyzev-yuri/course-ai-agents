...
## Цель урока
Перенести pipeline в контролируемую state machine.

## Входное состояние проекта
Файлы, которые ученик должен открыть перед занятием: `src/news_impact/langgraph_pipeline.py`.
...
## Ожидаемый результат
Понятны ingest -> dedup -> score nodes и fallback без langgraph.
